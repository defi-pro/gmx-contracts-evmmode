...
contract PositionRouter is BasePositionManager, IPositionRouter {
    ...
    function executeIncreasePositions(uint256 _endIndex, address payable _executionFeeReceiver) external override onlyPositionKeeper {
        ...
        while (index < _endIndex) {
            ...
            // Moved to _executeIncreasePositions_aux() to avoid zksolc compiler error
            if(!_executeIncreasePositions_aux(key,_executionFeeReceiver)){
                break;
            }
    ...
    function _executeIncreasePositions_aux(bytes32 key,address payable _executionFeeReceiver)  private returns (bool){
    ...
    function executeDecreasePositions(uint256 _endIndex, address payable _executionFeeReceiver) external override onlyPositionKeeper {
        ...
        while (index < _endIndex) {
            ...
            // Moved to _executeIncreasePositions_aux() to avoid zksolc compiler error
            if(!_executeDecreasePositions_aux(key,_executionFeeReceiver)){
                break;
            }      
    ...
    function _executeDecreasePositions_aux(bytes32 key,address payable _executionFeeReceiver)  private returns (bool){
